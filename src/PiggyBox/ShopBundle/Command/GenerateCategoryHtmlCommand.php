		...
		foreach ($shops as $shop) {
			...
			foreach($categories as $category){
				if(!$categories_buffer->contains($category))
				{
					...
					$html.='<li class=""><a href="'.$this->getContainer()->get('router')->generate('user_show_shop',array('slug' => $shop->getSlug(), 'category_title' => $category->getTitle(), '_format' => 'json')).'" class="category-item '.$category->getTitle().'">'.$category->getTitle().'</a></li>';
					...
					foreach ($children_categories as $children_category) {
						...
						{
								$html.='<li class=""><a href="'.$this->getContainer()->get('router')->generate('user_show_shop',array('slug' => $shop->getSlug(), 'category_title' => $children_category->getTitle(), '_format' => 'json')).'" class="category-item '.$children_category->getTitle().' ">'.$children_category->getTitle().'</a></li>';
