...
class ProductRepository extends EntityRepository
{
    ...
    {
        return $this->getEntityManager()
            ->createQuery('SELECT p FROM PiggyBoxShopBundle:Product p WHERE (p.category=:category_id AND p.shop=:shop_id AND NOT p.id=:product_id AND p.active=1)')
            ->setParameters(array(
                'shop_id' => $shop_id,
                'category_id' => $category_id,
    ...
    {
        return $this->getEntityManager()
            ->createQuery('SELECT p FROM PiggyBoxShopBundle:Product p WHERE (p.shop=:shop_id AND NOT p.category=:category_id AND p.active=1)')
            ->setParameters(array(
                'shop_id' => $shop_id,
                'category_id' => $category_id,
                ))
